...
router.post('/', [auth, upload.fields([{ name: 'file', maxCount: 1 }, { name: 'paymentScreenshot', maxCount: 1 }])], async (req, res) => {
    try {
        ...
        const totalCost = pricePerPage * numPages * copies;
        ...
        const newOrder = new Order({
            ...
            totalCost,
            paymentMethod: 'upi', // Force UPI or use req.body.paymentMethod if we still support others
...
router.post('/draft', [auth, upload.single('file')], async (req, res) => {
    try {
        ...
        // Basic price calc using pdf-parse could be done elsewhere; placeholder here
        const pricePerPage = printType === 'bw' ? 2 : 10;
        const numPages = 5; // placeholder; frontend may already provide pages
        const totalCost = pricePerPage * numPages * parseInt(copies, 10);

        const newOrder = new Order({
            userId: req.user.id,
            filePath: orderFile.path,
            printSettings: { printType, copies, paperSize, pickupTime },
            totalCost,
